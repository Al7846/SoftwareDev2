...
public:
	...
	void RoomAction()
	{
		...
		for (const auto& d : map_of_rooms) {//looping through map
			...
			{
				int index;//stores index value of the character position whether the string starts from
				string a = d.second;//stores the map key value as a temporary string
				while ((index = a.find(current_mode)) != string::npos) {//locates the starting index position of the room mode substring contained in the string value, 'npos' means no string matches have been found and is used in the while loop as a condition to iteratively match every charcter until a full match has been found. This link helped me understand 'npos'. https://www.educative.io/answers/what-is-stringnpos-in-cpp Accessed at: 6/1/23
...
public:
	...
	string temp;//stores each text file line as a single string in the temp variable
